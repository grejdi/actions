...
task_run=$(aws ecs run-task --task-definition "${ECS_TASK_DEFINITION}" --cluster "${ECS_CLUSTER}" --launch-type FARGATE  --network-configuration "awsvpcConfiguration={subnets=[${VPC_SUBNET}],securityGroups=[${VPC_SECURITY_GROUP}],assignPublicIp=${VPC_AUTO_ASSIGN_IP}}")
# get unique task arn
