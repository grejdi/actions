...
if [[ -z "${ECS_TASK_OVERRIDES}" ]]; then # if not defined
  task_run=$(aws ecs run-task --task-definition "${ECS_TASK_DEFINITION}" --cluster "${ECS_CLUSTER}" --launch-type FARGATE --network-configuration "${ECS_TASK_NETWORK_CONFIGURATION}")
else
